@echo off
echo ========================================
echo Installing GitSync CMD Wrapper Service
echo ========================================
echo.
...
echo Installing CMD wrapper service...
sc create "GitSync-Service" binPath= "cmd.exe /c C:\1C-CI-CD\ci\scripts\gitsync-service.cmd" DisplayName= "GitSync 1C Synchronization Service" start= auto

if %errorLevel% == 0 (
    echo CMD wrapper service installed successfully.
    ...
    echo.
    echo CMD wrapper service installation completed!
    
) else (
    echo Failed to install CMD wrapper service.
    pause
    exit /b 1
)
