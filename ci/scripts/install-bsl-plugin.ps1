...
try {
    # Использование System.Net.WebClient для более надежной загрузки
    $webClient = New-Object System.Net.WebClient
    $webClient.Headers.Add("User-Agent", "PowerShell")
    
    # Показ прогресса
    $progressPreference = 'SilentlyContinue'
    $webClient.DownloadFile($pluginUrl, $pluginFile)
...
# Предложение автоматического перезапуска
$restart = Read-Host "Перезапустить SonarQube сейчас? (y/n)"
if ($restart -eq 'y') {
    Write-Host ""
